...
void newPin(bool allowSkipping, u32 pinMode)
{
    ...
    drawString(true, 10, 10, COLOR_TITLE, "Inserisci un nuovo PIN usando ABXY e il DPAD");
    drawString(true, 10, 10 + SPACING_Y, COLOR_TITLE, allowSkipping ? "Premi START per saltare, SELECT per resettare" : "Premi SELECT per resettare");

    drawFormattedString(true, 10, 10 + 3 * SPACING_Y, COLOR_WHITE, "PIN (%u cifre): ", length);
    ...
    if(!fileWrite(&pin, PIN_FILE, sizeof(PinData)))
        error("Errore nella scrittura del file PIN");
}
