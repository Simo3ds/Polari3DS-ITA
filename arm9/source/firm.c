...
void loadHomebrewFirm(u32 pressed)
{
    ...
    if(payloadSize <= 0x200 || !checkFirm(payloadSize)) error("La payload e' invalida o corrotta.");
...
u32 patchTwlFirm(u32 firmVersion, FirmwareSource nandType, bool loadFromStorage, bool doUnitinfoPatch)
{
    ...
    //Apply EmuNAND patches
    if(nandType != FIRMWARE_SYSNAND) ret += patchEmuNand(process9Offset, process9Size, firmVersion, true);
