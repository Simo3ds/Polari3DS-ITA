...
void chainloader(void)
{
	...
	Path_Firm* firm = (Path_Firm*)MAP_BASE_1;
	...
	if (FSUSER_OpenDirectory(&handle, sdmcArchive, PathFirm) != 0) {
		Draw_DrawString(10, 60, COLOR_RED, "Nessun percorso file");
		Draw_FlushFramebuffer();
		Draw_Unlock();
		waitInputWithTimeout(0);
		return;
	...
	while(true)
	{
		...
		Draw_DrawString(10, 30, COLOR_WHITE, "Premi A per caricare Firm, premi B per ritornare");
		for(int i = 0; i < count; i++)
		{
			Draw_DrawString(30, 60+(i*10), COLOR_BLACK, "                                                ");
			Draw_DrawFormattedString(30, 60+(i*10), (i+pos == index) ? COLOR_TITLE : COLOR_WHITE, "   %s",firm->fileNames[i+pos]);
			if(i+pos== index)Draw_DrawString(30, 60+(i*10),COLOR_TITLE, "=>");
			if(i==12)break;
		}
		Draw_FlushFramebuffer();
		Draw_Unlock();
		...
		if(pressed & KEY_A){
			...
			if(copy_bootonce(sdmcArchive, firm->Path) == 0)svcKernelSetState(7);
...
u32 copy_bootonce(FS_ArchiveID sdmcArchive, const char *Path)
{
	...
	if (FSUSER_OpenFile(&fileHandle, sdmcArchive, fsMakePath(PATH_ASCII, Path), FS_OPEN_READ, 0) != 0)
        {
		Draw_DrawString(10, 60, COLOR_RED, "errore nell'apertura del file !");
		Draw_FlushFramebuffer();
		Draw_Unlock();
		waitInputWithTimeout(0);
		return 1;
	...
	if (FSUSER_OpenFile(&fileHandle2, sdmcArchive, fsMakePath(PATH_ASCII, "bootonce.firm"), FS_OPEN_CREATE | FS_OPEN_WRITE, 0) != 0)
        {
		Draw_DrawString(10, 60, COLOR_RED, "errore nell'apertura/scrittura del file !");
		Draw_FlushFramebuffer();
		Draw_Unlock();
