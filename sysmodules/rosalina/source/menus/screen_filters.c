...
Menu screenFiltersMenu = {
    "Menu filtri schermo",
    {
        { "[6500K] Temperatura predefinita", METHOD, .method = &ScreenFiltersMenu_SetDefault },
        { "[10000K] Aquario", METHOD, .method = &ScreenFiltersMenu_SetAquarium },
        { "[7500K] Oltre cielo", METHOD, .method = &ScreenFiltersMenu_SetOvercastSky },
        ...
        { "[2300K] Molto Incandescente", METHOD, .method = &ScreenFiltersMenu_SetWarmIncandescent },
        { "[1900K] Candela", METHOD, .method = &ScreenFiltersMenu_SetCandle },
        { "[1200K] Ambra", METHOD, .method = &ScreenFiltersMenu_SetEmber },
        { "Configurazioni avanzate", METHOD, .method = &ScreenFiltersMenu_AdvancedConfiguration },
        {},
    }
};
...
static u32 ScreenFiltersMenu_AdvancedConfigurationHelper(const ScreenFilter *filter, int offset, int pos, u32 posY)
{
    ...
    Draw_DrawCharacter(10, posY, COLOR_TITLE, pos == offset++ ? '>' : ' ');
    posY = Draw_DrawFormattedString(30, posY, COLOR_WHITE, "Luminosità:  %13s    \n", buf);

    Draw_DrawCharacter(10, posY, COLOR_TITLE, pos == offset++ ? '>' : ' ');
    posY = Draw_DrawFormattedString(30, posY, COLOR_WHITE, "Inverti:      %13s    \n", filter->invert ? "true" : "false");
...
void ScreenFiltersMenu_AdvancedConfiguration(void)
{
    ...
    {
        Draw_Lock();
        Draw_DrawString(10, 10, COLOR_TITLE, "Menu filtri schermo");

        posY = 30;
        posY = Draw_DrawString(10, posY, COLOR_WHITE, "Usa sinistra/destra per aumen./dim. il val. selezionato\n");
        posY = Draw_DrawString(10, posY, COLOR_WHITE, "Mantieni R per cambiare il val. piu' velocemente.\n");
        posY = Draw_DrawFormattedString(10, posY, COLOR_WHITE, "Aggiorna tutti e due gli schermi: %s (L per impost.)   \n", sync ? "si" : "no") + SPACING_Y;

        posY = Draw_DrawString(10, posY, COLOR_WHITE, "Schermo super.:\n");
        posY = ScreenFiltersMenu_AdvancedConfigurationHelper(&topScreenFilter, 0, pos, posY) + SPACING_Y;

        posY = Draw_DrawString(10, posY, COLOR_WHITE, "Schermo infer.:\n");
