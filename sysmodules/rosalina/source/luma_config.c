...
static size_t LumaConfig_SavePerformanceLumaIniConfigToStr(char *out, const CfgData *cfg, bool homeBrew, bool maxAppMem, bool core2)
{
    char lumaVerStr[64];
    char lumaRevSuffixStr[16];
    char rosalinaMenuComboStr[128];
    ...
    autobootModeStr = homeBrew ? "3ds" : "off";
    ...
    int n = sprintf(
        ...
        (int)cfg->formatVersionMajor, (int)cfg->formatVersionMinor,
        (int)CONFIG(AUTOBOOTEMU), (int)CONFIG(LOADEXTFIRMSANDMODULES),
        (int)CONFIG(PATCHGAMES), core2 ? 1 : 0,
        (int)CONFIG(PATCHVERSTRING), (int)CONFIG(SHOWGBABOOT),
        (int)CONFIG(PERFORMANCEMODE), (int)CONFIG(ALLOWUPDOWNLEFTRIGHTDSI),
        (int)CONFIG(CUTWIFISLEEP), (int)CONFIG(PATCHUNITINFO),
        (int)CONFIG(DISABLEARM11EXCHANDLERS), (int)CONFIG(ENABLESAFEFIRMROSALINA),
        ...
        cfg->autobootTwlTitleId, maxAppMem ? 1 : 0,
...
Result LumaConfig_SaveSettings(void)
{
    ...
    if (n > 0)
        res = IFile_Open(&file, archiveId, fsMakePath(PATH_EMPTY, ""), fsMakePath(PATH_ASCII, "/luma/lumae.ini"), FS_OPEN_CREATE | FS_OPEN_WRITE);
    else
        res = -1;
...
Result LumaConfig_SavePerformanceSettings(bool homeBrew, bool maxAppMem, bool core2)
{
    ...
    size_t n = LumaConfig_SavePerformanceLumaIniConfigToStr(inibuf, &configData, homeBrew, maxAppMem, core2);
    FS_ArchiveID archiveId = isSdMode ? ARCHIVE_SDMC : ARCHIVE_NAND_RW;
    if (n > 0)
        res = IFile_Open(&file, archiveId, fsMakePath(PATH_EMPTY, ""), fsMakePath(PATH_ASCII, "/luma/lumae.ini"), FS_OPEN_CREATE | FS_OPEN_WRITE);
    else
